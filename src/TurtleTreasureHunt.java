...
public class TurtleTreasureHunt implements KeyEventDispatcher {
	...
	private void spaceBarWasPressed() {
		...
		if (tortoiseLocationX > 490 && tortoiseLocationX < 550 && tortoiseLocationY > 280 && tortoiseLocationY < 340){
			JOptionPane.showMessageDialog(null, "Go to the Hat!");
		}
		 else if (tortoiseLocationX >= 390 && tortoiseLocationX <= 415 && tortoiseLocationY >= 240 && tortoiseLocationY <= 245){
			 JOptionPane.showMessageDialog(null, "You completed what ever your trying to do!");
		 }
	...
	private void go() {
		...
		int height = Tortoise.getBackgroundWindow().getHeight();
		int width = Tortoise.getBackgroundWindow().getWidth();
